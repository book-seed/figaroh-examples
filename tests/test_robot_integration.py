...
class TestRobotExampleIntegration:
    ...
    def test_tiago_example_with_unified_config(self):
        ...
        try:
            ...
            parser_variant = UnifiedConfigParser(
                config_file, variant="mocap_calibration"
            )
            config_variant = parser_variant.parse()
            cal_config_variant = create_task_config(
                robot, config_variant, "calibration"
            )
    ...
    def test_staubli_identification_with_unified_config(self):
        ...
        staubli_config = {
            ...
            "tasks": {
                "identification": {
                    ...
                    "data": {"trajectory_type": "fourier", "excitation_duration": 10.0},
                }
            },
        }
        ...
        try:
            ...
            assert (
                id_config["parameters"]["estimation_method"] == "weighted_least_squares"
            )
    ...
    def test_talos_dual_arm_configuration(self):
        ...
        talos_config = {
            ...
            "variants": {
                "right_arm_calibration": {
                    "tasks": {
                        "calibration": {
                            "kinematics": {"tool_frame": "arm_right_7_joint"},
                            "measurements": {
                                "markers": [
                                    {
        ...
        try:
            ...
            parser_right = UnifiedConfigParser(
                config_file, variant="right_arm_calibration"
            )
            ...
            assert (
                cal_config_right["measurements"]["markers"][0]["name"]
                == "right_arm_marker"
            )

            # Test dual arm variant
            parser_dual = UnifiedConfigParser(
                config_file, variant="dual_arm_calibration"
            )
            ...
            marker_names = [
                m["name"] for m in cal_config_dual["measurements"]["markers"]
            ]
        ...
        for robot_spec in robot_configs:
            ...
            test_config = {
                "robot": {
                    ...
                    "properties": {
                        "joints": {
                            "active_joints": [
                                f"joint_{i}" for i in range(robot_spec["joints"])
                            ]
                        }
                    },
                },
            ...
            try:
                ...
                if "joints" in config["robot"]["properties"]:
                    active_joints = config["robot"]["properties"]["joints"][
                        "active_joints"
                    ]
