...
class TestUR10UnifiedConfig:
    ...
    def test_ur10_config_loading(self, config_path):
        ...
        try:
            ...
            assert (
                "shoulder_pan_joint"
                in config["robot"]["properties"]["joints"]["active_joints"]
            )
...
class TestTIAGoUnifiedConfig:
    ...
    def test_tiago_variants(self):
        ...
        try:
            ...
            config = parser.parse()
            assert (
                config["tasks"]["calibration"]["kinematics"]["base_frame"] == "universe"
            )
            assert (
                config["tasks"]["calibration"]["parameters"]["outlier_threshold"]
                == 0.05
            )

            # Test mocap variant
            parser_variant = UnifiedConfigParser(
                test_config_path, variant="mocap_calibration"
            )
            config_variant = parser_variant.parse()
            assert (
                config_variant["tasks"]["calibration"]["kinematics"]["base_frame"]
                == "head_2_joint"
            )
            assert (
                config_variant["tasks"]["calibration"]["parameters"][
                    "outlier_threshold"
                ]
                == 0.01
            )
...
class TestTALOSUnifiedConfig:
    ...
    def test_talos_config_loading(self):
        ...
        try:
            ...
            reduction_ratios = config["robot"]["properties"]["mechanics"][
                "reduction_ratios"
            ]
...
class TestStaubliTX40UnifiedConfig:
    ...
    def test_staubli_config_loading(self):
        ...
        simplified_config = {
            ...
            "tasks": {
                "calibration": {
                    "enabled": True,
                    "type": "kinematic_calibration",
                    "kinematics": {"base_frame": "universe", "tool_frame": "joint_6"},
                    "measurements": {
                        "markers": [
                            {
        ...
        try:
            ...
            assert len(config["robot"]["properties"]["joints"]["active_joints"]) == 6
            assert all(
                joint.startswith("joint_")
                for joint in config["robot"]["properties"]["joints"]["active_joints"]
            )

            # Verify SCARA-specific properties (higher wrist speeds)
            velocities = config["robot"]["properties"]["joints"]["joint_limits"][
                "velocity"
            ]
    ...
    def test_staubli_identification_task(self, mock_robot):
        ...
        assert (
            task_config["parameters"]["estimation_method"] == "weighted_least_squares"
        )
            ...
            for variant_name, variant_config in variants.items():
                ...
                try:
                    ...
                    if robot_name == "ur10" and variant_name == "minimal_calibration":
                        assert (
                            cal_task["parameters"]["calibration_level"]
                            == "joint_offset"
                        )
    ...
    def test_robot_specific_validation(self):
        ...
        for robot_spec in robot_configs:
            test_config = {
                "robot": {
                    ...
                    "properties": {
                        "joints": {
                            "active_joints": [
                                f"joint_{i}" for i in range(robot_spec["joint_count"])
                            ]
                        }
                    },
                },
            ...
            try:
                ...
                assert (
                    len(config["robot"]["properties"]["joints"]["active_joints"])
                    == robot_spec["joint_count"]
                )
