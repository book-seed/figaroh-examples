...
class TestBackwardCompatibility:
    ...
    def test_unified_format_parsing(self):
        ...
        unified_config = {
            ...
            "tasks": {
                "calibration": {
                    ...
                    "data": {"number_of_samples": 200, "sampling_rate": 125.0},
                },
                "identification": {
                    "enabled": True,
                    ...
                    "data": {"sampling_rate": 1000.0, "filter_cutoff": 50.0},
                },
            },
            "environment": {"gravity": [0, 0, -9.81], "temperature": 22.0},
            "custom": {"manufacturer": "Test Robotics", "model": "TR-3000"},
        }
        ...
        try:
            ...
            assert (
                id_config["parameters"]["estimation_method"]
    ...
    def test_mixed_format_handling(self):
        ...
        mixed_config = {
            ...
            "robot": {
                "name": "mixed_robot",
                "properties": {"joints": {"active_joints": ["j1", "j2", "j3"]}},
            },
        }
    ...
    def _compare_calibration_configs(self, legacy_result, unified_result):
        ...
        if "parameters" in legacy_result and "parameters" in unified_result:
            legacy_params = legacy_result["parameters"]
            unified_params = unified_result["parameters"]

            # Compare outlier threshold
            if (
                "outlier_threshold" in legacy_params
                and "outlier_threshold" in unified_params
            ):
            ...
            if (
                "calibration_level" in legacy_params
                and "calibration_level" in unified_params
            ):
        ...
        if "kinematics" in legacy_result and "kinematics" in unified_result:
            legacy_kin = legacy_result["kinematics"]
            unified_kin = unified_result["kinematics"]

            # Compare frames
            if "base_frame" in legacy_kin and "base_frame" in unified_kin:
