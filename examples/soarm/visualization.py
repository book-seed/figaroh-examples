...
def add_axis_to_frame(gui, data, model, frame, is_joint=False):
    """Add coordinate axes to a frame."""
    if is_joint:
        gui.createGroup(frame)
        # gui.addToGroup(frame, "world")
        placement = pin.SE3ToXYZQUATtuple(data.oMi[model.getJointId(frame)])
...
robot = load_robot("urdf/SO100/so100.urdf", load_by_urdf=True)
...
add_axis_to_frame(gui, data, model, 'universe')
