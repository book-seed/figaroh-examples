...
class TX40Identification(BaseIdentification):
    ...
    def load_trajectory_data(self) -> dict:  # type: ignore[override]
        ...
        try:
            ...
            if curr_data.shape != pos_data.shape:
                msg = (
                    f"Data size mismatch: curr_data has {curr_data.shape} "
    ...
    def _convert_motor_to_joint_positions(self):
        ...
        joint_positions = np.dot(
            np.linalg.inv(reduction_matrix), self.raw_data["positions"].T
        )
    ...
    def _sync_torque_w_kinematics(self):
        ...
        if self.processed_data["torques"].shape[0] != current_kinematic_length:
            ...
            min_length = min(
                self.processed_data["torques"].shape[0], current_kinematic_length
            )
            self.processed_data["torques"] = self.processed_data["torques"][
                :min_length, :
            ]

            # Also trim kinematic data to match
            kinematic_keys = ["timestamps", "positions", "velocities", "accelerations"]
            for key in kinematic_keys:
                if key in self.processed_data:
                    self.processed_data[key] = self.processed_data[key][:min_length, :]
    ...
    def add_additional_parameters(self) -> None:
        ...
        if self.identif_config.get("has_coupled_wrist", False):
            self.standard_parameter["Iam6"] = float(self.identif_config["Iam6"])
            self.standard_parameter["fvm6"] = float(self.identif_config["fvm6"])
            self.standard_parameter["fsm6"] = float(self.identif_config["fsm6"])
