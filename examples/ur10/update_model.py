...
ordered_names = ["shoulder", "upper_arm", "forearm", "wrist_1", "wrist_2", "wrist_3"]
template = """    x: $x
    y: $y
    z: $z
    roll: $roll
...
def update_parameters(f_input, f_output, var, calib_config):
    ...
    jMop = SE3(translation=var[-6:-3], rotation=pinocchio.rpy.rpyToMatrix(var[-3:]))
    # d435_mount_link in wrist_3_joint
    jMm = SE3(
        translation=np.array([0.0, 0.0, 0.0]),
        rotation=pinocchio.rpy.rpyToMatrix(np.array([0.0, 0.0, pi])),
    )
    # optical frame in ref_camera_link
    cMop = SE3(
        translation=np.array([0.011, 0.033, 0.013]),
