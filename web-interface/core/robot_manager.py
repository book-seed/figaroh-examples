...
class RobotManager:
    ...
    def load_robot_with_method(
        self, loader: str, source: str, add_fext: bool = False, **kwargs
    ) -> bool:
        ...
        try:
            ...
            if cache_key in self.loaded_robots:
                ...
                print(
                    f"✅ Robot '{source}' loaded from cache " f"using '{loader}' loader"
                )
                return True
            ...
            print(f"❌ Failed to load robot '{source}' " f"with loader '{loader}': {e}")
            self.robot = None
            self.robot_info = {"status": "failed", "error": str(e)}
            return False

    def load_robot_from_paths(
        self, urdf_path: str, model_path: str, loader: str = "figaroh", **kwargs
    ) -> bool:
        ...
        try:
            ...
            robot_obj = load_robot(
                urdf_file=urdf_path, loader=loader, package_dirs=model_path, **kwargs
            )
    ...
    def load_robot_model(self, urdf_file: str, **kwargs) -> bool:
        ...
        try:
            ...
            if load_robot_alt:
                self.robot = load_robot_alt(urdf_file, package_dirs=self.models_path)
                self.robot_info = {
    ...
    def is_robot_loaded(self) -> bool:
        ...
        return self.robot is not None and self.robot_info.get("status") == "loaded"
        ...
        try:
            ...
            if hasattr(self.robot, "validate_configuration"):
                if not self.robot.validate_configuration(configuration):
                    ...
                    return False
            elif hasattr(self.robot, "model") and hasattr(self.robot.model, "nq"):
                if len(configuration) != self.robot.model.nq:
                    print(
                        f"Configuration size mismatch: "
