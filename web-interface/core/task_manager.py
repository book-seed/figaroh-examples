class TaskManager:
    ...
    def execute_task(self, task_config: dict, progress_callback=None):
        ...
        if not os.path.exists(example_path):
            raise FileNotFoundError(f"Example directory {example_path} not found")

        # Update progress
        if progress_callback:
            progress_callback(
                {"progress": 0, "message": f"Starting {task_type} task..."}
            )
        ...
        try:
            ...
            if progress_callback:
                progress_callback(
                    {"progress": 20, "message": "Locating task script..."}
                )
            ...
            for script in task_scripts:
                ...
                if os.path.exists(script_path):
                    ...
                    if progress_callback:
                        progress_callback(
                            {"progress": 40, "message": f"Executing {script}..."}
                        )
    ...
    def _execute_script_with_progress(
        self, script, example_path, task_config, progress_callback
    ):
        ...
        progress_thread = threading.Thread(
            target=self._simulate_progress, args=(process, progress_callback)
        )
    ...
    def _simulate_progress(self, process, progress_callback):
        ...
        while process.poll() is None and not self.task_stopped:
            ...
            if progress_callback:
                progress_callback(
                    {"progress": progress, "message": "Processing task..."}
                )
