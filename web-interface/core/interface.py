...
class FigarohWebInterface:
    ...
    def setup_interface(self):
        ...
        self.status_text = self.server.gui.add_text(
            "Status", initial_value="🟢 Ready - Select an example to begin"
        )
    ...
    def setup_panels(self):
        ...
        with self.server.gui.add_folder("Setup"):
            ...
            with self.server.gui.add_folder("🤖 Robot Loading"):
                ...
                self.path_selection_folder = self.server.gui.add_folder(
                    "📁 Path Selection", visible=False
                )
            ...
            with self.server.gui.add_folder("📊 Data Management"):
                self.data_panel = DataPanel(server=self.server, debug=self.debug)

        # Task Panel
        with self.server.gui.add_folder("⚙️ Tasks"):
            self.task_panel = TaskPanel(
                server=self.server, task_manager=self.task_manager, debug=self.debug
            )

        # Results Panel
        with self.server.gui.add_folder("Review"):
            with self.server.gui.add_folder("📈 Results"):
                self.results_panel = ResultsPanel(server=self.server)
            # Visualization Panel
            with self.server.gui.add_folder("👁️ Visualization"):
                self.visualization_panel = VisualizationPanel(server=self.server)
    ...
    def setup_path_selection_panel(self):
        ...
        # URDF Path Selection
        urdf_options = (
            list(self.available_urdf_paths.keys())
        ...
        model_options = (
            list(self.available_model_paths.keys())
    ...
    def _discover_urdf_paths(self):
        ...
        if (
            hasattr(self, "robot_panel")
            and self.robot_panel
            and hasattr(self.robot_panel, "source_dropdown")
            and self.robot_panel.source_dropdown.value != "None"
        ):
            ...
            for model_name in os.listdir(models_path):
                ...
                if os.path.isdir(model_dir):
                    ...
                    if selected_source:
                        ...
                        if not (
                            selected_source in model_name
                            or model_name in selected_source
                            or selected_source.replace("_description", "") in model_name
                            or model_name.replace("_description", "") in selected_source
                        ):
                            continue
        ...
        try:
            ...
            if selected_key != "None":
                if selected_key in self.available_urdf_paths:
                    self.selected_urdf_path = self.available_urdf_paths[selected_key]
        ...
        try:
            ...
            if selected_key != "None":
                if selected_key in self.available_model_paths:
                    self.selected_model_path = self.available_model_paths[selected_key]
    ...
    def _on_refresh_paths(self, _):
        ...
        try:
            ...
            print(
                f"Refreshed paths: {len(urdf_options)} URDF files, "
    ...
    def _load_robot_from_selected_paths(self):
        ...
        try:
            ...
            if not self.selected_model_path:
                ...
                self.selected_model_path = os.path.dirname(self.selected_urdf_path)
            ...
            if hasattr(self.robot_panel, "load_robot_from_paths"):
                self.robot_panel.load_robot_from_paths(
                    self.selected_urdf_path, self.selected_model_path
                )
            else:
    ...
    def setup_3d_scene(self):
        ...
        self.server.scene.add_grid(
            "/ground", width=6, height=6, cell_size=0.5, cell_thickness=0.01
        )
    ...
    def on_robot_loaded(self, robot_info):
        ...
        try:
            ...
            self.status_text.value = (
                f"🟢 Robot loaded: {robot_info.get('name', 'Unknown')}"
            )
    ...
    def on_data_loaded(self, data_info):
        ...
        try:
            ...
            self.status_text.value = (
                f"🟢 Data loaded: {success_count}/{total_count} files"
            )
    ...
    def on_data_selected(self, selection_info):
        ...
        try:
            ...
            if action == "added":
                ...
                self.status_text.value = (
                    f"📁 Added {filename} ({selected_count} files selected)"
                )
            elif action == "removed":
                filename = selection_info.get("file", "")
                self.status_text.value = (
                    f"� Removed {filename} ({selected_count} files selected)"
                )
            elif action == "cleared":
    ...
    def on_data_validated(self, validation_result):
        ...
        if validation_result["valid"]:
            ...
            self.status_text.value = (
                f"🟡 Data validation warnings: {len(validation_result['warnings'])}"
            )
