...
class FigarohWebApp:
    ...
    def initialize(self):
        ...
        try:
            ...
            # Initialize managers
            self.example_loader = ExampleLoader(example_path=self.examples_path)

            self.robot_manager = RobotManager(models_path=self.models_path)
            ...
            if hasattr(self, "interface") and self.interface:
                ...
                if self.use_streamlined:
                    ...
                    if (
                        hasattr(self.interface, "setup_panel")
                        and self.interface.setup_panel
                    ):
                        self.interface.setup_panel.update_examples(examples)
                        print(
                            f"🔄 Updated streamlined interface with {len(examples)} examples"
                        )
                else:
    ...
    def run(self):
        ...
        try:
            ...
            print(
                f"📊 Ready to serve {len(self.example_loader.get_all_examples())} examples"
            )
