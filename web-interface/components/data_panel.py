...
class DataPanel:
    ...
    def setup_ui(self):
        ...
        try:
            ...
            self.data_info_text = self.server.gui.add_text(
                "Data Info", initial_value="No data files available", disabled=True
            )
            ...
            self.validation_text = self.server.gui.add_text(
                "Validation Status", initial_value="No data loaded", disabled=True
            )
    ...
    def _on_clear_files(self, _):
        ...
        try:
            ...
            if self.on_data_selected:
                self.on_data_selected(
                    {"action": "cleared", "file": None, "selected_files": []}
                )
    ...
    def _update_data_info(self, filename=None):
        ...
        try:
            ...
            if self.selected_data_files:
                info_lines.append(
                    f"✅ {len(self.selected_data_files)} file(s) selected"
                )
    ...
    def _on_load_data(self, _):
        ...
        try:
            ...
            success_count = sum(
                1 for f in loaded_data.values() if f.get("loaded_successfully")
            )
            total_count = len(self.selected_data_files)

            if success_count == total_count:
                self.validation_text.value = (
                    f"✅ All {total_count} files loaded successfully"
                )
            else:
                self.validation_text.value = (
                    f"⚠️ {success_count}/{total_count} files loaded successfully"
                )
