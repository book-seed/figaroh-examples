...
class TaskPanel:
    ...
    def __init__(
        self, server: viser.ViserServer, task_manager=None, debug: bool = False
    ):
    ...
    def setup_ui(self):
        ...
        try:
            ...
            self.task_status = self.server.gui.add_text(
                "Task Status", initial_value="No task selected"
            )

            # Task parameters (will be populated based on task type)
            self.task_params_folder = self.server.gui.add_folder("📝 Parameters")
    ...
    def setup_calibration_config(self):
        ...
        with self.task_config_folder:
            ...
            self.calib_iterations = self.server.gui.add_number(
                "Max Iterations", initial_value=100, min=1, max=1000, step=1
            )

            self.calib_tolerance = self.server.gui.add_number(
                "Tolerance", initial_value=1e-6, min=1e-12, max=1e-2, step=1e-7
            )

        with self.task_params_folder:
            self.use_regularization = self.server.gui.add_checkbox(
                "Use Regularization", initial_value=True
            )

            self.regularization_weight = self.server.gui.add_slider(
                "Regularization Weight", min=0.0, max=1.0, step=0.01, initial_value=0.1
            )
    ...
    def setup_identification_config(self):
        ...
        with self.task_config_folder:
            ...
            self.use_base_params = self.server.gui.add_checkbox(
                "Use Base Parameters", initial_value=True
            )

            self.qr_tolerance = self.server.gui.add_number(
                "QR Tolerance", initial_value=1e-8, min=1e-12, max=1e-4, step=1e-9
            )

        with self.task_params_folder:
            self.filter_data = self.server.gui.add_checkbox(
                "Filter Data", initial_value=True
            )

            self.filter_cutoff = self.server.gui.add_slider(
                "Filter Cutoff (Hz)", min=1.0, max=100.0, step=1.0, initial_value=10.0
            )
    ...
    def setup_optimization_config(self):
        ...
        with self.task_config_folder:
            ...
            self.max_iter = self.server.gui.add_number(
                "Max Iterations", initial_value=1000, min=10, max=10000, step=10
            )

        with self.task_params_folder:
            self.use_bounds = self.server.gui.add_checkbox(
                "Use Parameter Bounds", initial_value=True
            )
    ...
    def setup_trajectory_config(self):
        ...
        with self.task_config_folder:
            ...
            self.duration = self.server.gui.add_slider(
                "Duration (s)", min=1.0, max=60.0, step=1.0, initial_value=10.0
            )

            self.frequency = self.server.gui.add_slider(
                "Frequency (Hz)", min=0.1, max=10.0, step=0.1, initial_value=1.0
            )

        with self.task_params_folder:
            self.joint_limits = self.server.gui.add_checkbox(
                "Respect Joint Limits", initial_value=True
            )

            self.smooth_trajectory = self.server.gui.add_checkbox(
                "Smooth Trajectory", initial_value=True
            )
    ...
    def setup_execution_config(self):
        ...
        with self.task_params_folder:
            self.save_results = self.server.gui.add_checkbox(
                "Save Results", initial_value=True
            )
    ...
    def setup_generic_config(self):
        ...
        with self.task_params_folder:
            self.verbose = self.server.gui.add_checkbox(
                "Verbose Output", initial_value=True
            )
    ...
    def collect_task_config(self) -> Dict[str, Any]:
        ...
        if task_type == "calibration":
            config.update(
                {
                    "method": (
                        getattr(self, "calib_method", None).value
                        if hasattr(self, "calib_method")
                        else "Kinematic"
                    ),
                    "max_iterations": (
                        getattr(self, "calib_iterations", None).value
                        if hasattr(self, "calib_iterations")
                        else 100
                    ),
                    "tolerance": (
                        getattr(self, "calib_tolerance", None).value
                        if hasattr(self, "calib_tolerance")
                        else 1e-6
                    ),
                    "use_regularization": (
                        getattr(self, "use_regularization", None).value
                        if hasattr(self, "use_regularization")
                        else True
                    ),
                    "regularization_weight": (
                        getattr(self, "regularization_weight", None).value
                        if hasattr(self, "regularization_weight")
                        else 0.1
                    ),
                }
            )
            ...
            config.update(
                {
                    "identification_type": (
                        getattr(self, "ident_type", None).value
                        if hasattr(self, "ident_type")
                        else "Dynamic"
                    ),
                    "use_base_params": (
                        getattr(self, "use_base_params", None).value
                        if hasattr(self, "use_base_params")
                        else True
                    ),
                    "qr_tolerance": (
                        getattr(self, "qr_tolerance", None).value
                        if hasattr(self, "qr_tolerance")
                        else 1e-8
                    ),
                    "filter_data": (
                        getattr(self, "filter_data", None).value
                        if hasattr(self, "filter_data")
                        else True
                    ),
                    "filter_cutoff": (
                        getattr(self, "filter_cutoff", None).value
            ...
            config.update(
                {
                    "method": (
                        getattr(self, "optim_method", None).value
                        if hasattr(self, "optim_method")
                        else "L-BFGS-B"
                    ),
                    "max_iterations": (
                        getattr(self, "max_iter", None).value
                        if hasattr(self, "max_iter")
                        else 1000
                    ),
                    "use_bounds": (
                        getattr(self, "use_bounds", None).value
                        if hasattr(self, "use_bounds")
                        else True
                    ),
                    "ftol": (
                        getattr(self, "ftol", None).value
                        if hasattr(self, "ftol")
                        else 1e-9
                    ),
                }
            )
            ...
            config.update(
                {
                    "trajectory_type": (
                        getattr(self, "traj_type", None).value
                        if hasattr(self, "traj_type")
                        else "Random"
                    ),
                    "duration": (
                    ...
                    "frequency": (
                        getattr(self, "frequency", None).value
                        if hasattr(self, "frequency")
                        else 1.0
                    ),
                    "joint_limits": (
                        getattr(self, "joint_limits", None).value
                        if hasattr(self, "joint_limits")
                        else True
                    ),
                    "smooth_trajectory": (
                        getattr(self, "smooth_trajectory", None).value
                        if hasattr(self, "smooth_trajectory")
                        else True
                    ),
                }
            )
            ...
            config.update(
                {
                    "mode": (
                        getattr(self, "execution_mode", None).value
                        if hasattr(self, "execution_mode")
                        else "Simulation"
                    ),
                    "save_results": (
                        getattr(self, "save_results", None).value
            ...
            config.update(
                {
                    "verbose": (
                        getattr(self, "verbose", None).value
    ...
    def start_task_execution(self, task_config: Dict[str, Any]):
        ...
        self.task_thread = threading.Thread(
            target=self._execute_task_thread, args=(task_config,)
        )
    ...
    def _execute_task_thread(self, task_config: Dict[str, Any]):
        ...
        try:
            ...
            results = self.task_manager.execute_task(
                task_config, progress_callback=self._task_progress_callback
            )
