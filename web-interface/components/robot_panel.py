...
class RobotPanel:
    ...
    def __init__(self, server: viser.ViserServer, example_loader=None, interface=None):
        ...
        self.interface = (
            interface  # Reference to main interface for accessing selected paths
        )
        self.robot_models = {}
    ...
    def setup_ui(self):
        ...
        try:
            ...
            available_loader_names = [
                name
                for name, info in self.available_loaders.items()
            ...
            self.robot_info = self.server.gui.add_text(
                "Robot Info", initial_value="No robot loaded", disabled=True
            )
    ...
    def _get_loader_info(self, loader_name: str) -> str:
        ...
        if loader_name in self.available_loaders:
            ...
            status = (
                "✅ Available" if info.get("available", False) else "❌ Not available"
            )
    ...
    def _on_source_selected(self, _):
        ...
        try:
            ...
            if self.interface and hasattr(self.interface, "update_paths_for_source"):
                self.interface.update_paths_for_source()
            elif self.interface and hasattr(
    ...
    def _on_loading_mode_changed(self, _):
        ...
        try:
            ...
            if self.interface and hasattr(
                self.interface, "_update_path_selection_visibility"
            ):
    ...
    def load_robot_combined(self):
        ...
        try:
            if self.loading_mode_dropdown.value == "Manual":
                ...
                print(
                    f"***************Loading robot with mode: {self.loading_mode_dropdown.value}"
                )
                ...
                if selected_source != "None" and load_robot:
                    self.load_robot_with_method(
                        selected_loader, selected_source, add_fext
                    )
                else:
        ...
        try:
            ...
            if loader == "robot_description":
                robot_obj = load_robot(
                    robot_urdf=source, loader="robot_description", isFext=add_fext
                )
    ...
    def load_robot_from_paths(self, urdf_path: str, model_path: str):
        ...
        try:
            ...
            adjusted_model_path = self._adjust_package_dirs_for_loader(
                model_path, selected_loader
            )
            robot_obj = load_robot(
            ...
            self._store_path_loaded_robot(
                robot_obj, robot_name, urdf_path, selected_loader
            )

        except Exception as e:
            self._handle_robot_loading_error(e, "robot from paths")

    def _validate_path_loading_inputs(
        self, urdf_path: str, model_path: str, loader: str
    ):
    ...
    def _store_path_loaded_robot(
        self, robot_obj, robot_name: str, urdf_path: str, loader: str
    ):
        ...
        try:
            ...
            if example_path:
                ...
                if urdf_path:
                    return {
                        "urdf_path": urdf_path,
                        "package_dirs": self._get_intelligent_package_dirs(
                            selected_source
                        ),
                    }
            ...
            if selected_source:
                ...
                if os.path.exists(models_path):
                    for model_name in os.listdir(models_path):
                        ...
                        if os.path.isdir(model_dir) and self._is_source_match(
                            selected_source, model_name
                        ):
        ...
        try:
            ...
            try:
                ...
                with self.server.gui.add_folder(
                    f"🤖 Joint Control for {self.current_robot}"
                ):
                    try:
                ...
                print(
                    "✓ Robot displayed in 3D scene using ViserUrdf with interactive controls"
                )
                return

            except ImportError:
                print(
                    "ViserUrdf not available - robot visualization requires ViserUrdf"
                )
    ...
    def get_current_robot_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "loader": getattr(
                self, "loader_dropdown", type("", (), {"value": "unknown"})
            ).value,
        }
