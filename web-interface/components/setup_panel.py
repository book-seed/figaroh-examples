...
class SetupPanel:
        ...
        try:
            ...
            with self.server.gui.add_folder("📂 Project Setup"):
                ...
                self.setup_status = self.server.gui.add_text(
                    "Status", initial_value="Select an example to begin", disabled=True
                )
            ...
            with self.server.gui.add_folder("🤖 Robot Configuration"):
                ...
                self.manual_robot_folder = self.server.gui.add_folder(
                    "Manual Selection", visible=False
                )
                ...
                with self.manual_robot_folder:
                    ...
                    self.manual_loader_dropdown.on_update(
                        self._on_manual_loader_changed
                    )

                    # URDF path selection
                    self.urdf_dropdown = self.server.gui.add_dropdown(
                        "📄 URDF File", options=["None"], initial_value="None"
                    )
                    self.urdf_dropdown.on_update(self._on_urdf_changed)

                    # Model path selection
                    self.model_dropdown = self.server.gui.add_dropdown(
                        "🎯 Model Path", options=["None"], initial_value="None"
                    )
            ...
            with self.server.gui.add_folder("📊 Data Management"):
                ...
                with self.server.gui.add_folder("File Selection"):
                    ...
                    self.selected_data_text = self.server.gui.add_text(
                        "Selected Files", initial_value="None selected", disabled=True
                    )
                ...
                self.data_info = self.server.gui.add_text(
                    "Data Status", initial_value="No data loaded", disabled=True
                )
            ...
            with self.server.gui.add_folder("⚡ Quick Actions"):
                ...
                self.reset_button = self.server.gui.add_button("🔄 Reset All")
        ...
        try:
            ...
            if self.example_loader:
                ...
                if selected_name in examples:
                    ...
                    self.current_example_path = self.example_loader.get_example_path(
                        selected_name
                    )
    ...
    def _find_example_robot_files(self):
        ...
        if not self.current_example_path or not os.path.exists(
    ...
    def _on_add_data_file(self, _):
        ...
        try:
            ...
            if (
                selected_file
                not in ["Select a file...", "No data files found", "No data directory"]
                and selected_file not in self.selected_data_files
            ):
    ...
    def _update_selected_data_display(self):
        ...
        if not self.selected_data_files:
            ...
            self.selected_data_text.value = (
                f"{len(self.selected_data_files)} files: {files_text}"
            )
