...
class ConfigPanel:
    ...
    def _create_calibration_widgets(self):
        ...
        self.calib_param_folder = self.server.gui.add_folder(
        ...
        with self.calib_param_folder:
            ...
            if "outlier_eps" in calib_config:
                ...
                self.server.gui.add_markdown(
                    "Threshold for outlier detection in meters"
                )
            ...
            if "nb_sample" in calib_config:
                ...
                self.server.gui.add_markdown(
                    "Number of data samples to use for calibration"
                )

    def _create_identification_widgets(self):
        """Create widgets for identification parameters."""
        ident_config = self.current_config["identification"]
        self.iden_param_folder = self.server.gui.add_folder(
        ...
        with self.iden_param_folder:
            if "problem_params" in ident_config and ident_config["problem_params"]:
                ...
                self.config_widgets["has_friction"] = self.server.gui.add_checkbox(
                    "Include Friction",
                    initial_value=prob_params.get("has_friction", False),
                )
                ...
                self.config_widgets["has_joint_offset"] = self.server.gui.add_checkbox(
                    "Include Joint Offset",
                    initial_value=prob_params.get("has_joint_offset", False),
                )
                self.server.gui.add_markdown("Estimate joint position offsets")

                self.config_widgets["has_actuator_inertia"] = (
                    self.server.gui.add_checkbox(
                        "Include Actuator Inertia",
                        initial_value=prob_params.get("has_actuator_inertia", False),
                    )
                )
    ...
    def _create_general_widgets(self):
        ...
        with self.general_param_folder:
            ...
            for key, value in self.current_config.items():
                if key not in ["calibration", "identification"] and isinstance(
        ...
        try:
            ...
            if "calibration" in self.current_config:
                ...
                if "calib_level" in self.config_widgets:
                    calib_config["calib_level"] = self.config_widgets[
                        "calib_level"
                    ].value
                ...
                if "outlier_eps" in self.config_widgets:
                    calib_config["outlier_eps"] = self.config_widgets[
                        "outlier_eps"
                    ].value

                if "nb_sample" in self.config_widgets:
                    calib_config["nb_sample"] = int(
                        self.config_widgets["nb_sample"].value
                    )
            ...
            if "identification" in self.current_config:
                ...
                if "problem_params" in ident_config and ident_config["problem_params"]:
                    ...
                    if "has_friction" in self.config_widgets:
                        prob_params["has_friction"] = self.config_widgets[
                            "has_friction"
                        ].value
                    if "has_joint_offset" in self.config_widgets:
                        prob_params["has_joint_offset"] = self.config_widgets[
                            "has_joint_offset"
                        ].value
                    if "has_actuator_inertia" in self.config_widgets:
                        prob_params["has_actuator_inertia"] = self.config_widgets[
